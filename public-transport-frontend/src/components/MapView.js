...
export default function RouteOptimizeQuickTest() {
  ...
  <button
    ...
    style={{
      padding: '6px 12px',
      borderRadius: 6,
      background: sortBy === 'TIME' ? '#4f46e5' : '#e5e7eb',
      color: sortBy === 'TIME' ? '#fff' : '#111',
      border: 'none'
    }}>
    🕐 Nhanh nhất
  </button>
  <button
    ...
    style={{
      padding: '6px 12px',
      borderRadius: 6,
      background: sortBy === 'DISTANCE' ? '#4f46e5' : '#e5e7eb',
      color: sortBy === 'DISTANCE' ? '#fff' : '#111',
      border: 'none'
    }}>
    📏 Ngắn nhất
  </button>
  <button
    ...
    style={{
      padding: '6px 12px',
      borderRadius: 6,
      background: sortBy === 'TRANSFERS' ? '#4f46e5' : '#e5e7eb',
      color: sortBy === 'TRANSFERS' ? '#fff' : '#111',
      border: 'none'
    }}>
    🔁 Ít trạm nhất
  </button>
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      map.on('load', () => {
        ...
        for (const leg of itinerary.legs) {
          const [startLat, startLng] = leg.polyline[0];
          const [endLat, endLng] = leg.polyline[leg.polyline.length - 1];
        ...
        // Trạm trung chuyển đặc biệt (ví dụ: Rmit)
        if (itinerary.transferStops && Array.isArray(itinerary.transferStops)) {
          for (const stopName of itinerary.transferStops) {
            for (const leg of itinerary.legs) {
              const [startLat, startLng] = leg.polyline[0];
              const [endLat, endLng] = leg.polyline[leg.polyline.length - 1];

              if (leg.endStop === stopName) {
                new goongjs.Marker({ color: '#a855f7' }) // màu tím
                  .setLngLat([endLng, endLat])
                  .setPopup(new goongjs.Popup().setText(`Trạm trung chuyển: ${stopName}`))
                  .addTo(map);
              } else if (leg.startStop === stopName) {
                new goongjs.Marker({ color: '#a855f7' }) // màu tím
                  .setLngLat([startLng, startLat])
                  .setPopup(new goongjs.Popup().setText(`Trạm trung chuyển: ${stopName}`))
                  .addTo(map);
